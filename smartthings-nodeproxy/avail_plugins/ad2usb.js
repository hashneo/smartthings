...
function AD2USB () {
    ...
    let zones = nconf.get('ad2usb:panelConfig:zones');

    zones.forEach( (zone) => {
        panel.zones[zone.zone] = '';
    });
    ...
    this.init = function() {
        ...
        if (mode === 'serial') {
            ...
            device.setKeepAlive(true,20000);
            device.setTimeout(60000);

            device.on('timeout', () => {
                logger("AD2USB socket timeout");
                device.end();
    ...
    function keypad_update(data) {
        ...
        if (msg.alpha.toLowerCase().indexOf('hit * for faults') !== -1 && msg.userOrZone > 0 ) {
            logger('Exception condition - we have faults so system is not ready');
            msg.dscCode = 'NOT_READY';
        }
        // exception conditition - should never be DISARMED and not READY
        else if (msg.alpha.startsWith('DISARMED') && msg.dscCode !== 'READY') {
        ...
        if (msg.dscCode === '' || msg.dscCode === 'NOT_READY') {
